...
#' neighbours <- get_neighbours(data, plot)

#'
#'
get_neighbours <- function (df, plot_ID) {
    ...
    neighbours <- full_join(targets, targets, by = c("zzz", "plot"),
                           suffix = c("_target", "_neighbour")) %>%
        select(-zzz) %>%
        mutate(dist=sqrt((x_neighbour - x_target)^2 +
                             (y_neighbour - y_target)^2) )
